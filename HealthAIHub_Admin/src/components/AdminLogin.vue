...
import type { AdminLoginDto } from '@/service/api'
  ...
  await formRef.value.validate(async (valid, fields) => {
    if (valid) {
      try {
        ...
        const loginSuccess = await authStore.loginAdmin(loginData)
        if (loginSuccess) {
          ElMessage.success('登录成功')
          console.log('开始跳转到 /app/user-management')
          try {
            await router.push('/app/user-management')
            console.log('路由跳转完成')
          } catch (error) {
