  ...
  <div class="profile-view">
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item label="用户名">
        <el-input :value="authStore.currentAdmin?.username" disabled />
      </el-form-item>
...
import type { AdminUpdateDto } from '@/service/api'
import {
  profileEmailRules,
...
const form = ref<AdminUpdateDto & { confirmPassword: string }>({
  email: authStore.currentAdmin?.email || '',
  oldPassword: '',
  newPassword: '',
...
const avatarUrl = ref(authStore.currentAdmin?.avatar || '')
const isAvatarChanged = ref(false)
const selectedFile = ref<File | null>(null)

const originalEmail = ref(authStore.currentAdmin?.email || '')
...
const uploadAvatar = async () => {
  ...
  try {
    const result = await authStore.uploadAdminAvatar(selectedFile.value)
    if (result) {
  ...
  await formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      try {
        ...
        const updatedAdmin = await authStore.updateProfile(updateData)
        if (updatedAdmin) {
